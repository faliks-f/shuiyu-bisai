...
def search_circle(img, pipe_blob):
    global circle_roi, find_stop_flag_to_circle, has_seen
    # 当前点已经报过了，直接结束
    if find_stop_flag_to_circle or not has_seen:
        return None
    ...
    if min(w, h) / max(w, h) > 0.3 and w * h < 2500:
        # print(min(w, h) / max(w, h))
        # print(w * h)
        return max_b
...
def analysis(c):
    ...
    if (i == 5 or i == 4) and not allow_flag:
        return
    # 没有报过这个点
    if find_circle_flag == False:
        ...
        # 找到点的帧数加一
        find_circle_times += 1
        # 通过面积就可以知道是潜水还是深水，深水的点比较小，这样就有不同的max_times
        if c.w() * c.h() < 500 and not max_flag:
            max_times = 4
            max_flag = True
        else:
            max_times = 2
        # 当找到点的帧数达到max_times时，汇报这个点
        if find_circle_times == max_times:
            find_circle_flag = True
            find_circle_times = 0
            i += 1
            print("find", i)
            find_point_command[1] = i
            send(find_point_command)
            # 如果是深水的那两个点，就打开一个0.5s的定时器，通过将allow_flag修改为False，可以让鱼0.5s内不会汇报点，实际比赛时，这个值应该更大
            # 因为当时如果把0.5s改为1s，那么就可以满分通过了，不会误报那个点
            if i == 5 or i == 4:
                allow_flag = False
                tim = Timer(2, freq=2)
                tim.callback(handle_timer)
...
def stop(img):
    ...
    # print(min(w, h) / max(w, h))
    # 判断是否满足黑线的条件
    if min(w, h) / max(w, h) < 0.4 and h * w > 1500 and h * w < 3000 and w < 100:
        # print(w)
        return True, b
    return False, b
...
# 读取和写入的文件名
if test_read:
    img_reader = image.ImageReader("/stream8.bin")
if test_write:
...
while (True):
    ...
    if flag:
        ...
        # 这个记录帧数的，看到一帧+1，没看到-1，和之前那些没看到就归0的不同
